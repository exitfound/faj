...
- [Содержание:](Fully-Automated-Jenkins)
  - [Мотивация](Мотивация)
  - [[Структура репозитория](Структура-репозитория)](https://github.com/exitfound/faj?tab=readme-ov-file#%D0%BC%D0%BE%D1%82%D0%B8%D0%B2%D0%B0%D1%86%D0%B8%D1%8F)
...
- Ansible, если вы собираетесь развертывать Jenkins на двух или более серверах с разными ОС.
...
- **jenkins-docker-compose.yaml:** Файл для непосредственного запуска Jenkins на базе образа, который был собран с помощью Dockerfile.
...
- **.env:** Файл, содержащий список переменных, которые прописаны в конфигурационных файлах для плагина CasC. В общей сложности здесь отмечены самые часто изменяемые параметры в Jenkins. Например такие, как данные пользователя, общие настройки и т.д. Работает только в сочетании с классическим запуском Jenkins. При взаимодействии с Ansible используются файлы из `group_vars`, в которых определен тот же механизм взаимодействия. 

- **jcasc:** Директория с непосредственно конфигурационными файлами, которые используются для описания Jenkins в виде кода. В них же, вместо жесткой привязки какого-либо значения, указана переменная, ссылающаяся на файл `.env`. Работает только в сочетании с классическим запуском Jenkins. При взаимодействии с Ansible используются файлы из `group_vars`, в которых определен тот же механизм взаимодействия. 
